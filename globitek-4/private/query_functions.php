<?php
  ...
  function validate_user($user, $errors=array()) {
    ...
    if (is_blank($user['username'])) {
     ...
     if (is_blank($user['password'])|| is_blank($user['confirmPassword'])) {
      ...
      $errors[] = "Password must be between 12 to 255 characters.";
  ...
  function insert_failed_login($failed_login){
     ...
     $sql.= "(username, count, last_attempt) VALUES (";
  ...
  function update_failed_login($failed_login){
     ...
     $sql.= "WHERE id ='".db_escape($db,$failed_login['id'])."' LIMIT 1;";
  	 $result = db_query($db, $sql);
  	 return $result;
  }
  
  function login_counter($user){
      global $db;
